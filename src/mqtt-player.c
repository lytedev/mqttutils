...
#define CONF_DEFAULT_USERNAME NULL
#define CONF_MAX_LENGTH_USERNAME 256
	char username[CONF_MAX_LENGTH_USERNAME];

#define CONF_DEFAULT_PASSWORD ""
#define CONF_MAX_LENGTH_PASSWORD 512
	char password[CONF_MAX_LENGTH_PASSWORD];
...
void print_usage(char *progname) {
	...
	printf("-u --username       The username to use when connecting to the broker.\n");
	printf("-P --password       The password for the user to use when connecting to the broker.\n");
...
void parse_args(int argc, char **argv) {
	...
	for(i = 1; i < argc; i++) {
		...
		if( !strcmp(argv[i], "-c") || !strcmp(argv[i], "--client-id") ) {
			...
			if( ++i == argc ) {
				fprintf(stderr, "ERROR: Parameter %s given but no username specified.\n", argv[i-1]);
				print_usage(*argv);
				exit(1);
			} else {
				strncpy(config.username, argv[i], CONF_MAX_LENGTH_USERNAME);
			}

			// PASSWORD
		} else if( !strcmp(argv[i], "-P") || !strcmp(argv[i], "--password") ) {
			if( ++i == argc ) {
				fprintf(stderr, "ERROR: Parameter %s given but no password specified.\n", argv[i-1]);
				print_usage(*argv);
				exit(1);
			} else {
				strncpy(config.password, argv[i], CONF_MAX_LENGTH_PASSWORD);
			}
...
int main(int argc, char **argv) {
	...
	mosquitto_username_pw_set(config.mosq, config.username, config.password);
